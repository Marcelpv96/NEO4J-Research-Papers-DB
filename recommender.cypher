...
MATCH (kw:KeyWord)<-[:HasKeyWord]-(kw_paper:Scientific_Paper)-[:publishedON]->(type)
WHERE type:Volume OR type:Proceeding AND kw.name = "data management" OR kw.name  = "indexing" OR kw.name = "data modeling" OR kw.name = "big data" OR kw.name = "data processing" or kw.name = "data storage" OR kw.name = "data.querying"
WITH type, COUNT(distinct all_papers) as all_number, COUNT(distinct kw_paper) as kw_papers_number
